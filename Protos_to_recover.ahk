...
global playerHWND 
playerPath = %A_AppData%\Spotify\%playerExe%,playerWinTitle = ahk_exe %playerExe% 
...
; TapTime / Prefix can now be set hererr
thm := new TapHoldManager(,,,"~"),thm.Add("LAlt", Func("openLauncher")),thm.Add("CapsLock", Func("sendMegaModifier"))
...
releaseAllModifiers() 
{
    ...
    ~ScrollLock::
        If (GetKeyState("ScrollLock", "T")){
            ...
            SoundPlay, %A_ScriptDir%\sounds\protos_off.mp3r
        }else  {
            Menu, Tray, Icon, %A_ScriptDir%\protos.png, 1
    ...
    isWindowVisible(WTitle){
        ...
        getCursorWindow(){
            ...
            If (InStr(pName, "explorer") or pName = "")
                Return -1
            Else
        Return WinUMID }activateCursorWindow(){
    ...
    CapsLock & v:: 
        if (InStr(Clipboard, "document.query") = 0){
            SendStr := RegExReplace(Clipboard, "^""(.+)""$", "$1"),SendStr := StrReplace(SendStr, """""", """",,-1)
            ;MsgBox, %Clipboard%, %SendStr%
            SendEvent {Raw} document.querySelector('%SendStr%')}Else{
            SendStr := StrReplace(Clipboard, """", """"""),SendStr := RegExReplace(SendStr, "document\.querySelector(?:All)?\('?([^\(\)]*)'\)", "$1")
            SendEvent "%SendStr%"
        }
    Return
            ...
            If  (WinActive(grp)){
                ...
                If  (WinActive(grp)){
                    ...
                    *Pause:: 
                    ...
                    CapsLock & c::Run, %browserExe%%browserREPL%
                                    ...
                                    CapsLock & t::
                                        ...
                                        if (!id) {
                                            ...
                                            *Pause:: 
                                            ...
                                            return
                                            #Backspace::PostMessage, 0x0112, 0xF170, % (i:=!i)?2:-1,, A ; << Press F1 to toggle On/Off
                                            #If
                                            #If, GetKeyState("ScrollLock", "T")
                                            SendMode Input
                                            ;Space::RButton
                                                ...
                                                if EWD_LButtonState = U  ; Button has been relseased, so drag is complete.
                                                {
                                                    ...
                                                    if EWD_LButtonState = U  ; Button has been relseased, so drag is complete.
                                                    {
                                                        ...
                                                        if EWD_EscapeState = D  ; Escape has been pressed, so drag is cancelled.
                                                        {
                                                            ...
                                                            return
                                                            yState, EWD_EscapeState, Escape, P
                                                            if EWD_EscapeState = D  ; Escape has been pressed, so drag is cancelled.
                                                            {
                                                                SetTimer, EWD_WatchMouse, off
                                                                WinMove, ahk_id %EWD_MouseWin%,, %EWD_OriginalPosX%, %EWD_OriginalPosY%
                                                                ...
                                                                if EWD_EscapeState = D  ; Escape has been pressed, so drag is cancelled.
                                                                {
                                                                    ...
                                                                    EWD_MouseStartX := EWD_MouseX  ; Update for the next timer-call to this subroutine.,EWD_MouseStartY := EWD_MouseY
                                                                    return              